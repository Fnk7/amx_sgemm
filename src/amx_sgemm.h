...
void amx_sgemm(float *A, float *B, float *C, uint64_t size)
{
    // limitation
    if (size == 0ull)
        return;
    ...
    {
        float *Ai = A + i * size;
        // for 2 * 16 columns of B
        for (uint64_t j = 0ull; j < size; j += 32ull)
        {
            float *Bj = B + j;
            /*  for all rows in the 16 columns of A, 
             *  and for all columns in the 16 rows of B
             *  
             *  like for all elements of a columns of A,
             *  for all elements of a rows of B,
