...
void transformA(const float *A, float *A0, uint64_t sizei, uint64_t sizek)
{
    for (uint64_t i = 0ull; i < sizei; i += 16ull)
    {
        ...
        {
            // load A[i:i+16][k:k+32]
            for (uint64_t ii = 0ull; ii < 16ull; ii++)
            {
                amx_ldz((uint8_t *)(&A[(i + ii) * sizek + k]), ii << 2, 1ull);
            }
            // transpose z to x, y
            uint64_t oprand_to_x = 0x8000000004004000;
            uint64_t oprand_to_y = 0x8000000010004000;
            for (uint64_t zoffset = 0ull; zoffset < 2ull; zoffset++)
            {
                for (uint64_t offset = 0ull; offset < 8ull; offset++)
                {
                    AMX_EXTRY(oprand_to_x | ((((offset) << 2) + zoffset) << 20) | (offset << 6));
                    AMX_EXTRY(oprand_to_y | ((((offset + 8ull) << 2) + zoffset) << 20) | (offset << 6));
                }
...
void _amx_sgemm(float *A, float *B, float *C, const uint64_t sizei, const uint64_t sizej, const uint64_t sizek)
{
    ...
    if (sizei == 0ull || sizek == 0ull || sizej == 0ull)
        return;
    if (sizei % 32 != 0 || sizek % 32 != 0 || sizej % 64 != 0)
        return;
    // A0[sizei / 16][sizek][16]
    ...
    for (uint64_t i = 0ull; i < sizei; i += 16ull)
    {
        ...
        // 4 * 16 columns of B as a column tile
        for (uint64_t j = 0ull; j < sizej; j += 64ull)
        {
            float *Bj = B + j;
            for (uint64_t k = 0ull; k < sizek; k += 2ull)
            {
                // load A[i:i+16][k:k+2]
                amx_ldy((uint8_t *)(A0i + 16ull * k), 0ull, 1ull);
                // load B[k:k+2][j:j+64]
                amx_ldx((uint8_t *)(Bj + sizej * (k + 0ull) + 00ull), 0ull, 1ull);
                amx_ldx((uint8_t *)(Bj + sizej * (k + 0ull) + 32ull), 2ull, 1ull);
                amx_ldx((uint8_t *)(Bj + sizej * (k + 1ull) + 00ull), 4ull, 1ull);
                amx_ldx((uint8_t *)(Bj + sizej * (k + 1ull) + 32ull), 6ull, 1ull);
                for (uint64_t yoffset = 0ull; yoffset < 2ull; yoffset++)
                {
                    uint64_t zignore = k == 0ull && yoffset == 0ull ? 1ull : 0ull;
                    for (uint64_t zoffset = 0ull; zoffset < 4ull; zoffset++)
                    {
                        amx_fma32((yoffset << 2) + zoffset, yoffset, zoffset, zignore);
                    }
                }
            }
            ...
            for (uint64_t offset = 0; offset < 16ull; offset++)
            {
                ...
                amx_stz((uint8_t *)(Cij + sizej * offset + 32ull), (offset << 2) + 2ull, 1ull);
            }
        }
    }
    AMX_STOP();
