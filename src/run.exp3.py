...
def run(cmd, output):
    ...
    err += abs(os.system("./a.out > omp_result/exp3/{}".format(output)))
    time_end = time.time()
    ...
    for _ in range(31):
        ...
        if min_time > time_end - time_start:
            min_time = time_end - time_start
            err += abs(os.system("cp omp_result/tmp.txt omp_result/exp3/{}".format(output)))
    if err != 0:
        print("--- Failed! ---")
        exit(-1)
...
if __name__ == '__main__':
    ...
    for thread_num in [1, 2, 4, 8]:
        ...
        matrix_size_m = 512
        matrix_size_n = 1024
        for matrix_size_k in [256, 512, 512 + 256, 1024, 1024 + 512, 2048]:
